            ...
            <div className={!MenuCollapse ?  background + " fixed-top  " : background + " fixed-top text-center  " } style={!MenuCollapse ? styleMenuDefault : styleMenuMinim}>
                <div className="menus mt-1">
                    ...
                    <ul className="nav d-block mt-3 scroller" >{
                        listMenuSlice.filter(menu => menu.base===0).map(
                            (menu,index)=>
                            <li className="nav-item nav-sousmenu" data-bs-toggle="tooltip" data-bs-placement="right" title={menu.labelle_menu} key={index}>
                                <NavLink    className="btn btn-dark nav-link lien rounded-1 text-white d-flex justify-content-between"  
                                            to={menu.route} data-bs-toggle="collapse" data-bs-target={"#collapseMenu"+index} aria-expanded="false" aria-controls="collapseExample" > 
                                    {!MenuCollapse ? <span className="titleMenu "> {menu.labelle_menu}</span>:null}
                                ...
                                {menu && (
                                    <div className="collapse" id={"collapseMenu"+index}>
                                        <div className={!MenuCollapse ? background + " card card-body " :background+" card p-0 pt-1 text-center " }>
                                            {
                                                listMenuSlice.filter(sous_menu => sous_menu.base === menu.id_menu).map(
                                                    (sous_menu,index)=>
                                                    <NavLink    className="nav-link  lien rounded-1 mb-2 d-flex justify-content-between" 
                                                                to={"react_app/"+sous_menu.route} 
                                                                style={({ isActive }) => ({ background: isActive ? "#00000078" : "" , color: isActive ? "#7ee2a6" : "#ffffff" })} > 
                                                        {!MenuCollapse ? <span className= "titleMenu ms-2 d-inline" > {sous_menu.labelle_menu}</span>:null}
                                                        <i className={sous_menu.icon }></i>
                                                    </NavLink>  
                                                    {sous_menu && (
                                                        <div className="collapse" id={"collapseMenu"+index}>
                                                            <div className={!MenuCollapse ? background + " card card-body " :background+" card p-0 pt-1 text-center " }>
                                                                {
                                                                    listMenuSlice.filter(sous_sous_menu => sous_sous_menu.base === sous_menu.id_menu).map(
                                                                        (sous_sous_menu,index)=>
                                                                        <NavLink    className="nav-link  lien rounded-1 mb-2 d-flex justify-content-between" 
                                                                                    to={"react_app/"+sous_sous_menu.route} 
                                                                                    style={({ isActive }) => ({ background: isActive ? "#00000078" : "" , color: isActive ? "#7ee2a6" : "#ffffff" })} > 
                                                                            {!MenuCollapse ? <span className= "titleMenu ms-2 d-inline" > {sous_sous_menu.labelle_menu}</span>:null}
                                                                            <i className={sous_sous_menu.icon }></i>
                                                                        </NavLink>  
