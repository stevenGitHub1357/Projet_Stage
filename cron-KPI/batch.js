...
async function start() {
    ...
    cron.schedule('* * * * *',async()=>{	
        try{
            ...
            const getSuivieFNC_FAC = await db_lean.query("SELECT id_demande, titre, impact, statut, type_demande FROM suivis.suivi_fnc_fac");
            const performance = await db_kpi.query("SELECT id, id_base, type_demande, titre, objectif, statut, realise, date_demande, date_cloture, id_statut FROM revue_direction.performance");
            if(getSuivieFNC_FAC.length > 0){
                for(const item of getSuivieFNC_FAC){
                    var obj = {}
                    obj.id_base = item.id_demande;
                    ...
                    if(obj.type_demande.localeCompare("FNC")===0){
                        fiche = await db_lean.query("SELECT date_create, date_cloture, list_objectif FROM suivis.fiche_non_conformite WHERE id="+obj.id_base);
                        if(fiche.length>0){
                    ...
                    if(obj.type_demande.localeCompare("FAC")===0){
                        fiche = await db_lean.query("SELECT date_detection, date_cloture, list_objectif FROM suivis.fiche_amelioration_continue WHERE id="+obj.id_base);
                        if(fiche.length>0){
                    ...
                    if(fiche.length>0){
                        ...
                        let query = "";    
                        // let current = await db_kpi.query("SELECT id, type_demande, titre, objectif, statut, realise, date_demande, date_cloture, id_statut FROM data_kpi.fnc_fac WHERE id="+obj.id_base);
                        let current = performance.filter(perf => perf.id_base===obj.id_base && perf.type_demande === obj.type_demande)
                        // console.log(current)
